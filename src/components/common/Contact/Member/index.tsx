...
const MemberContactIcons = (props: MemberContactIconsProps) => {
  ...
  return (
    <Box
      ...
      marginTop="12px"
    >
      {props.contact.map((ct) => (
        <Tooltip key={ct.name} title={ct.name} aria-label={ct.label}>
          <Icon
            onClick={handleNavigation(ct.value)}
            className={cx(iconsMap[ct.label], props.className)}
          />
        </Tooltip>
      ))}
    </Box>
