...
const CareerRole = (props: CareerRoleProps) => {
    ...
    return <>
        ...
        <Dialog open={open} onClose={handleClose} PaperProps={{ style: { width: "40vw", padding: '40px' } }} maxWidth='lg'>
            <Box style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '45px' }}>
                <Box >
                    <Typography variant="h5" style={{ fontFamily: "'PT Mono', monospace", marginBottom: '4px', fontSize: '28px' }}>{props.role.role}</Typography>
            ...
            {props.role.requirements && <Box>
                <Typography style={{ marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '2px', opacity: '0.3' }}>Tech Stack</Typography>
                <Box style={{ height: '2px', background: '#fff', opacity: '0.1' }}></Box>
                <Box style={{ display: 'flex', flexWrap: 'wrap', paddingTop: '15px' }}>
                    {props.role.requirements.map(MapRequriements(props.role.requirements))}
                </Box>
            </Box>}
            <Box style={{ height: '2px', background: '#fff', opacity: '0.1', marginBottom: '12px' }}></Box>
            <DialogContent style={{ padding: '0' }}>
                <Typography style={{ opacity: '0.6', }}>{props.role.description}</Typography>
            </DialogContent>
            <DialogActions style={{ padding: '0', justifyContent: 'center', marginTop: '25px' }}>
...
const MapRequriements = (requirements: IRoleRequirements[]) => (requirement: IRoleRequirements) => (
    <Box style={{ margin: '0 35px 15px 0', }}>
        ...
        <form onSubmit={handleSubmit}>
            <Box display='flex' justifyContent='space-between'>
                ...
                <Box style={{ display: 'flex', alignItems: 'flex-start' }}>
                    <Button variant="text" onClick={handleCloseForm}><Icon>close</Icon></Button>
                </Box>
            </Box>
